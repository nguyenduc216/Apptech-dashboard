(function () {
    (() => {
        const lockHistoryState = { apptechNoBack: true, path: window.location.href };
        try {
            window.history.replaceState(lockHistoryState, document.title, window.location.href);
            window.history.pushState(lockHistoryState, document.title, window.location.href);
        } catch {
        }
    ...
    const createJsQrCodeInstance = (readerId) => {
            ...
            try {
                ...
                if (sourceWidth > 0 && sourceHeight > 0) {
                    ...
                    if (result?.data) {
                        ...
                        appendQrDebugLog("qr_jsqr_decode_candidate", {
                            width,
                            height,
        ...
        return {
            ...
            async start(cameraTarget, _config, onScanSuccess, onScanFailure) {
                ...
                const videoConstraintAttempts = buildVideoConstraints(cameraTarget);
                let lastPlayError = null;

                for (let attempt = 0; attempt < videoConstraintAttempts.length; attempt += 1) {
                    const constraints = {
                        video: videoConstraintAttempts[attempt],
                        audio: false
                    };

                    try {
                        appendQrDebugLog("qr_jsqr_getusermedia_start", { attempt: attempt + 1, ...constraints });
                        ...
                        appendQrDebugLog("qr_jsqr_attach_stream_before", { attempt: attempt + 1 });
                        video.srcObject = stream;
                        appendQrDebugLog("qr_jsqr_attach_stream_after", { attempt: attempt + 1 });
                        await waitNextFrame();
                        await waitNextFrame();

                        try {
                            await waitForVideoEvent("loadedmetadata", 1800);
                        } catch (error) {
                        ...
                        appendQrDebugLog("qr_jsqr_play_before", { attempt: attempt + 1 });
                        await playVideoWithTimeout();
                        appendQrDebugLog("qr_jsqr_play_after", { attempt: attempt + 1 });
                        lastPlayError = null;
                        break;
                    } catch (error) {
                        lastPlayError = error;
                        appendQrDebugLog("qr_jsqr_play_attempt_error", {
                            attempt: attempt + 1,
                            name: error?.name || "",
                            message: error?.message || `${error || ""}`
                        });
    ...
    const getQrResultPoints = (result) => {
        ...
        return Array.isArray(points)
            ? points
                .map((point) => ({
                    x: Number(point?.x ?? point?.X ?? Number.NaN),
                    y: Number(point?.y ?? point?.Y ?? Number.NaN)
                }))
                .filter((point) => Number.isFinite(point.x) && Number.isFinite(point.y))
            : [];
    };

    const showQrDetectedOutline = (reader, points = [], sourceWidth = 0, sourceHeight = 0) => {
        const panel = reader instanceof HTMLElement ? reader.closest(".qr-scanner-panel") : null;
        if (!(panel instanceof HTMLElement)) {
            return;
        }

        let outline = panel.querySelector(".qr-detected-outline");
        if (!(outline instanceof HTMLElement)) {
            outline = document.createElement("div");
            outline.className = "qr-detected-outline";
            panel.appendChild(outline);
        }

        const validPoints = Array.isArray(points) ? points : [];
        if (validPoints.length >= 2 && sourceWidth > 0 && sourceHeight > 0) {
            const xs = validPoints.map((point) => point.x);
            const ys = validPoints.map((point) => point.y);
            const minX = Math.max(0, Math.min(...xs) / sourceWidth * 100);
            const minY = Math.max(0, Math.min(...ys) / sourceHeight * 100);
            const maxX = Math.min(100, Math.max(...xs) / sourceWidth * 100);
            const maxY = Math.min(100, Math.max(...ys) / sourceHeight * 100);
            outline.style.left = `${minX}%`;
            outline.style.top = `${minY}%`;
            outline.style.width = `${Math.max(8, maxX - minX)}%`;
            outline.style.height = `${Math.max(8, maxY - minY)}%`;
        } else {
            outline.style.left = "18%";
            outline.style.top = "18%";
            outline.style.width = "64%";
            outline.style.height = "64%";
        }
    ...
    const startQrScannerCamera = async (html5QrCode, reader, cameraId, onScanSuccess, onScanFailure = () => {}) => {
        ...
        try {
                await html5QrCode.start(
                    ...
                    config,
                    (decodedText, result) => {
                        showQrDetectedOutline(reader, getQrResultPoints(result));
                        window.setTimeout(() => {
                            onScanSuccess(decodedText, result);
                        }, 180);
                    },
    ...
    document.querySelectorAll("[data-qr-scanner]").forEach((scanner, index) => {
        ...
        const duplicateAlert = scanner.querySelector("[data-qr-duplicate-alert]");
        ...
        const updateDuplicateAlert = (message = "") => {
            if (!(duplicateAlert instanceof HTMLElement)) {
                return;
            }

            const hasMessage = message.trim().length > 0;
            duplicateAlert.textContent = message;
            duplicateAlert.hidden = !hasMessage;
            duplicateAlert.classList.toggle("is-visible", hasMessage);

            if (hasMessage) {
                duplicateAlert.classList.remove("is-shaking");
                void duplicateAlert.offsetWidth;
                duplicateAlert.classList.add("is-shaking");
            } else {
                duplicateAlert.classList.remove("is-shaking");
            }
        };
        ...
        const tryAcceptQrValue = async (qrValue, successMessageBuilder) => {
            ...
            try {
                if (await isQrCodeInUse(normalizedValue)) {
                    const duplicateMessage = `Mã QR ${normalizedValue} đã tồn tại trên hệ thống. Vui lòng dùng mã khác.`;
                    setQrValue("");
                    updateDuplicateAlert("");
                ...
                updateDuplicateAlert("");
                updateStatus("Không kiểm tra được trạng thái mã QR trên hệ thống. Hãy thử lại.", "error");
                return false;
            }
        ...
        startButton.addEventListener("click", async () => {
            ...
            try {
                ...
                updateStatus("Không tải được thư viện quét QR. Bạn vẫn có thể nhập tay mã QR.", "error");
                isStarting = false;
                return;
            }
    ...
    (() => {
        ...
        startButton.addEventListener("click", async () => {
            ...
            try {
                ...
                updateStatus("Không tải được thư viện quét QR để tra cứu.", "error");
                isStarting = false;
                return;
            }
            ...
            try {
                ...
                await startQrScannerCamera(
                    ...
                    async (decodedText) => {
                        ...
                        if (matched) {
                            await stopScanner("Đang mở thông tin vật tư...");
                        }
                    },
                    () => {
                    });
                ...
                isRunning = false;
                setScannerMode("idle");
                updateStatus("Không thể truy cập camera hoặc khởi động bộ quét QR.", "error");
            } finally {
                isStarting = false;
            }
        });
    ...
    (() => {
        ...
        const updateStatus = (message, tone = "default") => {
            if (statusNode instanceof HTMLElement) {
                ...
                const isError = tone === "error";
                const statusShell = statusNode.closest("[data-xuat-kho-qr-status]") || statusNode;
                if (statusShell instanceof HTMLElement) {
                    statusShell.classList.toggle("is-error", isError);
                    ...
                    if (isError) {
                        statusShell.classList.remove("is-shaking");
                        void statusShell.offsetWidth;
                        statusShell.classList.add("is-shaking");
                        statusShakeTimeoutId = window.setTimeout(() => {
                            statusShell.classList.remove("is-shaking");
                            statusShakeTimeoutId = 0;
                        }, 450);
        ...
        const lookupAndAddQr = async (qrValue) => {
            const normalizedValue = `${qrValue || ""}`.trim();
            if (!normalizedValue) {
                updateStatus("Vui lòng scan hoặc nhập mã QR vật tư.", "error");
                return false;
            }
            ...
            try {
                ...
                if (!response.ok) {
                    updateStatus("Không kiểm tra được QR vật tư. Hãy thử lại.", "error");
                    return false;
                }

                const payload = await response.json();
                if (!payload?.found || !payload?.item) {
                    updateStatus(payload?.errorMessage || "Không tìm thấy vật tư còn tồn kho cho mã QR này.", "error");
                    return false;
                }
        ...
        form.addEventListener("submit", (event) => {
            ...
            if (rows.length === 0) {
                hasError = true;
                updateStatus("Vui lòng scan hoặc thêm ít nhất một vật tư cần xuất.", "error");
            }
