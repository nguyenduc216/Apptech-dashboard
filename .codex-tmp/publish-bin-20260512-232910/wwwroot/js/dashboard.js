(function () {
    ...
    const createQrCameraConfig = () => {
        const config = {
            ...
            qrbox: (viewfinderWidth, viewfinderHeight) => {
                ...
                const maxBox = isAppleMobileDevice() ? 580 : 500;
                const size = Math.max(260, Math.min(Math.floor(minEdge * 0.98), minEdge - 2, maxBox));
                return {
    ...
    const ensureQrScannerFrame = (reader) => {
        ...
        if (!(panel instanceof HTMLElement) || panel.querySelector(".qr-scanner-frame")) {
            return;
        }
    ...
    const startQrScannerCamera = async (html5QrCode, reader, cameraId, onScanSuccess, onScanFailure = () => {}) => {
        ...
        await html5QrCode.start(
            cameraId,
            createQrCameraConfig(),
            onScanSuccess,
            onScanFailure);
        await applyQrVideoEnhancements(html5QrCode);
    };
    ...
    const applyQrVideoEnhancements = async (html5QrCode) => {
        ...
        await tryApplyQrVideoConstraints(html5QrCode, {
            width: { ideal: isAppleMobileDevice() ? 3840 : 1920 },
            height: { ideal: isAppleMobileDevice() ? 2160 : 1080 },
        ...
        try {
            ...
            if (zoomFeature && typeof zoomFeature.isSupported === "function" && zoomFeature.isSupported()) {
                ...
                const target = clampNumber(isAppleMobileDevice() ? 1.25 : 1.9, min, max);
