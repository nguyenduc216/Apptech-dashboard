(function () {
    ...
    document.querySelectorAll("[data-cong-viec-checklist-manager-root]").forEach((form) => {
        ...
        const resetSortingStyles = (state) => {
            if (!state) {
                return;
            }

            state.item.classList.remove("is-hold-ready", "is-pointer-dragging");
            state.handle.classList.remove("is-hold-ready", "is-active");
            state.item.style.transform = "";
            state.item.style.zIndex = "";
        };

        const finishSorting = (commit) => {
            if (!sortingState) {
                return;
            }

            clearHoldTimer(sortingState);
            if (typeof sortingState.cleanup === "function") {
                sortingState.cleanup();
            }
            resetSortingStyles(sortingState);

            if (commit && sortingState.active) {
        ...
        const activatePointerSorting = (state, clientY) => {
            state.active = true;
            state.pointerOffsetY = clientY - state.item.getBoundingClientRect().top;
            state.item.classList.add("is-pointer-dragging");
            state.handle.classList.add("is-active");
        };

        const beginSortingAttempt = (item, handle, sourceKey, startClientX, startClientY) => {
            finishSorting(false);
            ...
            handle.classList.add("is-hold-ready");

            sortingState.holdTimerId = window.setTimeout(() => {
                if (!sortingState || sortingState.sourceKey !== sourceKey || sortingState.item !== item) {
                    return;
                }

                activatePointerSorting(sortingState, sortingState.latestClientY);
            }, 260);
        };

        const findReorderTarget = (dragItem, dragCenterY) => {
            const items = getItems().filter((item) => item !== dragItem);
        ...
        const updatePointerSorting = (clientY) => {
            if (!sortingState?.active) {
                return;
            }

            const listRect = list.getBoundingClientRect();
            const pointerYInList = clientY - listRect.top;
            const layoutTop = sortingState.item.offsetTop;
            const targetTop = pointerYInList - sortingState.pointerOffsetY;
            const deltaY = targetTop - layoutTop;
            const dragCenterY = listRect.top + targetTop + (sortingState.item.offsetHeight / 2);
            const targetItem = findReorderTarget(sortingState.item, dragCenterY);

            sortingState.item.style.transform = `translateY(${deltaY}px)`;

            if (targetItem instanceof HTMLElement) {
                list.insertBefore(sortingState.item, targetItem);
                return;
            }

            list.appendChild(sortingState.item);
        };
        ...
        const attachItemEvents = (item) => {
            ...
            if (dragHandle instanceof HTMLElement) {
                ...
                dragHandle.addEventListener("pointerdown", (event) => {
                    ...
                    beginSortingAttempt(item, dragHandle, `pointer:${event.pointerId}`, event.clientX, event.clientY);
                ...
                dragHandle.addEventListener("pointermove", (event) => {
                    if (!sortingState || sortingState.sourceKey !== `pointer:${event.pointerId}` || sortingState.item !== item) {
                        return;
                    }

                    sortingState.latestClientX = event.clientX;
                    sortingState.latestClientY = event.clientY;

                    if (!sortingState.active) {
                        const movedX = Math.abs(event.clientX - sortingState.startClientX);
                        const movedY = Math.abs(event.clientY - sortingState.startClientY);
                    ...
                    event.preventDefault();
                    updatePointerSorting(event.clientY);
                });

                const stopPointerSorting = (event, commit) => {
                    if (!sortingState || sortingState.sourceKey !== `pointer:${event.pointerId}` || sortingState.item !== item) {
                        return;
                    }

                    finishSorting(commit);
                };

                dragHandle.addEventListener("pointerup", (event) => {
                    stopPointerSorting(event, true);
                });

                dragHandle.addEventListener("pointercancel", (event) => {
                    stopPointerSorting(event, false);
                });
                ...
                dragHandle.addEventListener("touchstart", (event) => {
                    ...
                    const sourceKey = `touch:${touch.identifier}`;
                    beginSortingAttempt(item, dragHandle, sourceKey, touch.clientX, touch.clientY);

                    const handleTouchMove = (moveEvent) => {
                        const activeTouch = Array.from(moveEvent.touches).find((entry) => `touch:${entry.identifier}` === sourceKey);
                        if (!activeTouch || !sortingState || sortingState.sourceKey !== sourceKey || sortingState.item !== item) {
                            return;
                        }

                        sortingState.latestClientX = activeTouch.clientX;
                        sortingState.latestClientY = activeTouch.clientY;

                        if (!sortingState.active) {
                            const movedX = Math.abs(activeTouch.clientX - sortingState.startClientX);
                            const movedY = Math.abs(activeTouch.clientY - sortingState.startClientY);
                            if (movedX > 18 || movedY > 18) {
                                finishSorting(false);
                            }
                            return;
                        ...
                        updatePointerSorting(activeTouch.clientY);
                    };

                    const handleTouchEnd = (endEvent) => {
                        const releasedTouch = Array.from(endEvent.changedTouches).find((entry) => `touch:${entry.identifier}` === sourceKey);
                        if (!releasedTouch || !sortingState || sortingState.sourceKey !== sourceKey || sortingState.item !== item) {
                            return;
                        }
                    ...
                    const handleTouchCancel = (cancelEvent) => {
                        const cancelledTouch = Array.from(cancelEvent.changedTouches).find((entry) => `touch:${entry.identifier}` === sourceKey);
                        if (!cancelledTouch || !sortingState || sortingState.sourceKey !== sourceKey || sortingState.item !== item) {
                            return;
                        }
                    ...
                    if (sortingState && sortingState.sourceKey === sourceKey) {
                        sortingState.cleanup = cleanup;
                    }
                }, { passive: true });
            }
        };
    ...
    document.querySelectorAll("[data-qr-scanner]").forEach((scanner, index) => {
        ...
        const populateCameraOptions = (cameras) => {
            ...
            if (!selectedCameraId || !cameraOptions.some((camera) => camera.id === selectedCameraId)) {
                const preferredCamera = cameraOptions.find((camera) => {
        ...
        startButton.addEventListener("click", async () => {
            ...
            if (!html5QrCode) {
                html5QrCode = new window.Html5Qrcode(reader.id, false);
            }
    ...
    document.querySelectorAll("[data-status-toast]").forEach((toast) => {
