(function () {
    ...
    const qrScannerTuningVersion = "QR iOS v20260513.10";
    const qrDebugLogs = [];
    ...
    const createQrCameraTargets = (cameraId) => {
        ...
        if (cameraId) {
            return [
                { ...safeBase, deviceId: { ideal: cameraId } },
                { ...base, deviceId: { ideal: cameraId } },
                { ...safeBase, facingMode: { ideal: "environment" } },
    ...
    const startQrScannerCamera = async (html5QrCode, reader, cameraId, onScanSuccess, onScanFailure = () => {}) => {
        ...
        for (let index = 0; index < cameraTargets.length; index += 1) {
            const cameraTarget = cameraTargets[index];
            ...
            try {
                await html5QrCode.start(
                    ...
                    onScanSuccess,
                    onScanFailure);
                usedTarget = cameraTarget;
                appendQrDebugLog("qr_start_success", {
                    attempt: index + 1,
                    cameraTarget
                });
                break;
        ...
        if (!usedTarget) {
            throw lastStartError || new Error("Unable to start QR camera.");
        }
