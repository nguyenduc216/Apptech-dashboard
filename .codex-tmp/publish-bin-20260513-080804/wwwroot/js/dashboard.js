(function () {
    ...
    const qrScannerTuningVersion = "QR iOS v20260513.7";
    const qrDebugLogs = [];
    ...
    const getQrScannerProfile = () => {
        ...
        return {
            isApple,
            fps: isApple ? 24 : 24,
            qrboxRatio: isApple ? 0.995 : 0.985,
            minQrboxSize: isApple ? 250 : 260,
            maxQrboxSize: isApple ? 560 : 500,
    ...
    const ensureQrScannerFrame = (reader) => {
        ...
        let logButton = panel.querySelector(".qr-scanner-log-button");
    ...
    const startQrScannerCamera = async (html5QrCode, reader, cameraId, onScanSuccess, onScanFailure = () => {}) => {
        ...
        const cameraTarget = cameraId || { facingMode: { ideal: "environment" } };
        appendQrDebugLog("qr_start", {
            version: qrScannerTuningVersion,
            isApple: isAppleMobileDevice(),
            cameraId,
            cameraTarget,
            config: createQrCameraConfig(),
        ...
        const version = reader instanceof HTMLElement
            ...
            : null;
        if (version instanceof HTMLElement && runningSettings?.zoom) {
            const profile = getQrScannerProfile();
            version.textContent = `${qrScannerTuningVersion} | target ${profile.zoom}x | actual ${runningSettings.zoom}x`;
        }
    };
