(function () {
    ...
    const qrScannerTuningVersion = "QR iOS v20260513.9";
    const qrDebugLogs = [];
    ...
    const createQrCameraTarget = (cameraId) => {
        const profile = getQrScannerProfile();
        if (!profile.isApple) {
        ...
        if (cameraId) {
            target.deviceId = { exact: cameraId };
        } else {
    ...
    const startQrScannerCamera = async (html5QrCode, reader, cameraId, onScanSuccess, onScanFailure = () => {}) => {
        ...
        const cameraTarget = createQrCameraTarget(cameraId);
